...
for props in mobile_props:
    Mobd = {}
    Mobd["title"] = title =  props.find( name="div" , attrs={"class" : "_4rR01T"} ).get_text()
    Mobd["star_rating"]     = star_rating = props.find( name="div" , attrs={"class" : "_3LWZlK"} ).get_text()
    Mobd["ratings_reviews"] = ratings_reviews = props.find( name="span" , attrs={"class" : "_2_R_DZ"} ).get_text()
    mobile_specs  = props.find( name="ul" , attrs={"class" : "_1xgFaf"} )
    Mobd["price"]           = price  = props.find( name="div" , attrs={"class" : "_25b18c"} ).get_text()
    Mobd["delivery_date"]   = delivery_date = props.find( name="div" , attrs={"class" : "_3tcB5a p8ucoS"} ).get_text()
    Mobd["exchage_off"]     = exchage_off = props.find( name="div" , attrs={"class" : "_2ZdXDB"} ).get_text()
    
    specsl = []
    for specs in mobile_specs:
        specsl.append(specs.get_text())

    Mobd["mobile_Specs"]  = " , ".join(specsl)
